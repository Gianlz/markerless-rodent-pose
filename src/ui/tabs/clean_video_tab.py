...
class VideoCleanWorker(QThread):
    ...
    def __init__(
        self, videos: list[str], output_folder: str, codec: str, crf: int, preset: str
    ):
        super().__init__()
        self.videos = videos
    ...
    def run(self):
        try:
            ...
            for i, video in enumerate(self.videos):
                ...
                output_file = (
                    output_path / f"{video_path.stem}_clean{video_path.suffix}"
                )
...
class CleanVideoTab(QWidget):
    ...
    def init_ui(self):
        ...
        info_frame.setStyleSheet(
            "QGroupBox { border: none; background: transparent; margin-top: 0; padding: 0; }"
        )
    ...
    def check_integrity(self):
        ...
        for i in range(self.video_list.count()):
            ...
            if info:
                results.append(
                    f"✓ {video_name}: {info['width']}x{info['height']}, {info['fps']:.2f} fps"
                )
            else:
                results.append(
                    f"✗ {video_name}: Could not read metadata (possibly corrupted)"
                )

        self.set_busy(False)
        self.status_label.setText("Integrity check complete.")

        QMessageBox.information(self, "Video Integrity Check", "\n".join(results))
