...
class VideoPreviewDialog(QDialog):
    ...
    def update_image(self):
        ...
        cv2.line(
            display_frame,
            (self.line_x, 0),
            (self.line_x, display_frame.shape[0]),
            (0, 0, 255),
            2,
        )
    ...
    def update_image(self):
        ...
        for i in range(1, 4):
            ...
            if i < 3:
                cv2.line(
                    display_frame,
                    (line_pos_x, self.roi_y),
                    (line_pos_x, self.roi_y + self.roi_size),
                    (0, 255, 0),
                    1,
                )
            ...
            if i < 3:
                cv2.line(
                    display_frame,
                    (self.roi_x, line_pos_y),
                    (self.roi_x + self.roi_size, line_pos_y),
                    (0, 255, 0),
                    1,
                )
...
class TestWorker(QThread):
    ...
    def run(self):
        try:
            ...
            results = self.manager.calculate_box_sides_time(
                h5_file, self.video, self.line_x, fps
            )
...
class GridWorker(QThread):
    ...
    def __init__(
        self, manager: TestsManager, config: str, video: str, roi: tuple[int, int, int]
    ):
        super().__init__()
        self.manager = manager
        self.config = config
    ...
    def run(self):
        try:
            ...
            results = self.manager.calculate_grid_test(
                h5_file, self.video, self.roi, fps
            )
...
class TestsTab(QWidget):
    ...
    def set_grid_roi(self):
        ...
        if dialog.exec():
            ...
            self.grid_roi_label.setText(
                f"ROI: x={self.roi[0]}, y={self.roi[1]}, size={self.roi[2]}"
            )
    ...
    def run_box_test(self):
        ...
        if self.line_x is None:
            QMessageBox.warning(
                self, "Validation Error", "Please set the center line first."
            )
            return
    ...
    def on_box_finished(self, results: dict):
        ...
        self.result_a_lbl.setText(
            f"{results['time_A_sec']:.2f} seconds ({results['frames_A']} frames)"
        )
        self.result_b_lbl.setText(
            f"{results['time_B_sec']:.2f} seconds ({results['frames_B']} frames)"
        )

        total_sec = (
            results["total_frames"] / results["fps"] if results["fps"] > 0 else 0
        )
        self.result_total_lbl.setText(
            f"{total_sec:.2f} seconds ({results['total_frames']} frames @ {results['fps']:.1f} fps)"
        )
    ...
    def run_grid_test(self):
        ...
        if self.roi is None:
            QMessageBox.warning(
                self, "Validation Error", "Please set the Grid ROI first."
            )
            return
