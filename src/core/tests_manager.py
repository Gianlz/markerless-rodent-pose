...
class TestsManager:
    ...
    def analyze_video_if_needed(
        self, config_path: str, video_path: str, shuffle: int = 1
    ) -> str:
    ...
    def calculate_box_sides_time(
        self, h5_file: str, video_path: str, line_x: int, fps: float
    ) -> dict:
        """
        ...
        for i in range(total_frames):
            ...
            cv2.putText(
                frame,
                f"Side A (Left): {time_left_cur:.1f}s",
                (10, 30),
            ...
            cv2.putText(
                frame,
                f"Side B (Right): {time_right_cur:.1f}s",
                (w - 300, 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.8,
                (0, 255, 0),
            ...
            if current_side is None:
                color = (255, 255, 255)
            cv2.putText(
                frame, status_text, (10, h - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 3
            )
    ...
    def play_realtime_trace(
        self, h5_file: str, video_path: str, trail_length: int = 60, params: dict = None
    ):
        ...
        colors = [
            (int(c[2] * 255), int(c[1] * 255), int(c[0] * 255)) for c in colors_cm
        ]  # BGR
        ...
        while True:
            ...
            for bp_idx, bp in enumerate(bodyparts):
                ...
                if isinstance(likelihood, pd.Series):
    ...
    def play_realtime_detection(
        self, h5_file: str, video_path: str, params: dict = None
    ):
    ...
    def calculate_grid_test(
        ...
        for i in range(len(df)):
            ...
            if len(paws_in_squares) == len(valid_paws) and len(paws_in_squares) > 0:
                if (
                    all(sq == paws_in_squares[0] for sq in paws_in_squares)
                    and paws_in_squares[0] is not None
                ):
            ...
            status_text = (
                f"Square: {current_square}" if current_square else "Square: None"
            )
            cv2.putText(
                frame,
                status_text,
        ...
        ax.add_patch(
            plt.Rectangle(
                (rx, ry), rsize, rsize, fill=False, edgecolor="red", linewidth=2
            )
        )
        for step in range(1, 3):
        ...
        if heatmap_x and heatmap_y:
            ...
            heatmap_roi_pts = [
                (x, y)
                for x, y in zip(heatmap_x, heatmap_y)
                if rx <= x <= rx + rsize and ry <= y <= ry + rsize
            ]
            ...
            if heatmap_roi_pts:
                ...
                H, xedges, yedges = np.histogram2d(
                    hx, hy, bins=bins, range=[[rx, rx + rsize], [ry, ry + rsize]]
                )
                ...
                plt.imshow(
                    ...
                    cmap="jet",
                    extent=[rx, rx + rsize, ry + rsize, ry],
                    aspect="auto",
                )
        ...
        time_per_square = {
            sq: f / fps if fps > 0 else 0 for sq, f in frames_per_square.items()
        }
        ...
        logger.info(
            f"Saved grid test results to {excel_path}, {traj_img_path}, {heatmap_img_path}"
        )
        return results
