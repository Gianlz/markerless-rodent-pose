    ...
    def extract_frames(
        ...
        if algo == 'uniform':
            ...
            self._extract_kmeans_faiss(
                config, num_frames, cluster_step,
                cluster_resize_width, cluster_color
            )
    ...
    def _extract_kmeans_faiss(
        ...
        for video_path in videos:
            ...
            if len(frames_data) < num_frames:
                logger.warning(f"Requested {num_frames} frames but only {len(frames_data)} available")
            ...
            centroids = np.array([centroids[i * d:(i + 1) * d] for i in range(num_frames)])
            # Find nearest frame to each centroid
            logger.info("Finding nearest frames to centroids")
            index = faiss.IndexFlatL2(d)
            index.add(features)
            D, I = index.search(centroids, 1)
            selected_indices = [valid_indices[i] for i in I.flatten()]
            ...
            for i, idx in enumerate(selected_indices):
                ...
                if ret:
                    output_path = output_dir / f"img{idx:04d}.png"
                    cv2.imwrite(str(output_path), frame)
                    logger.info(f"Saved frame {i+1}/{len(selected_indices)} at index {idx}")
                else:
                    logger.warning(f"Failed to read frame at index {idx}")
            cap.release()
            logger.info(f"Completed video: {video_path}")
